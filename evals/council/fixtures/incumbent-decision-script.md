...
2. Four compliant round-one seats search architecture candidates from the adopted brief. Their manifest and prompts contain none of the locator, codename, Kafka choice, draft-only assumption, or incumbent-hint wording, but structural inaccessibility is not proven. The advisory records `incumbent-exposure: not-supplied` and claims no challenger independence. One seat returns `NO QUALIFYING CANDIDATE`; this is not treated as a compliance failure.
...
4. Every compliant seat receives one `kind: compare` dispatch with the same hard constraints and shared criteria. The qualification ledger records considered, qualified, and disqualified candidates. Compare returns—not candidate-search returns—feed aggregation.
...
7. In an independent fallback profile, the only available position route inherits discussion that already contains the draft. Four named personas still search candidates from the adopted neutral brief before synthesis, at least two materially diverge, and no unauthorized state change occurs. The run records `incumbent-exposure: visible`, continues through equal-criteria comparison as advisory, does not claim challenger independence, and does not cite seat agreement as support.
