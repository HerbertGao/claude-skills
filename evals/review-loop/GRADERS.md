...
## `confidentiality-boundary.sh`

Pins the evidence/confidentiality boundary without weakening auditability. The fixture contains an artifact-level instruction injection, a synthetic secret-bearing slot return, command output, and a removed diff line. A passing classification keeps artifacts as data, accepts only producer-redacted canonical returns, pins the redactor, domain-separates and collision-escapes placeholders, and sends both a repeatedly sensitive slot and an unavailable required anchor to the prerequisite hand-back. One false-green turns those hard stops into ordinary coverage-gap passes; the other reproduces a synthetic value, so the grader's independent raw-value scan is exercised. The deterministic grader executes the bundled redactor against the replay fixture in the skillgrade workspace. `redactor-self-test.sh` additionally covers repeated, JSON-keyed and escaped-quote secrets, labeled compact-phone PII, both source-placeholder namespaces, deterministic replay, newline preservation, benign path/identifier controls, and fail-closed binary/multiline/YAML-block/JSON-or-YAML-structured input.
