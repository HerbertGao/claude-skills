...
- 现有 `evals/review-loop/fixtures/mechanic-struct-*` 是**小 OUTCOME 级** fixture;**用它们之前先验基线是否真在其上 ~0/5**,否则同样测不出效果型改动的价值。
...
- [ ] 边际 ≈0 / 检查不触发 → **不加**,如实记录(别为「感觉有用」硬加,simplicity-lens 会咬)。
