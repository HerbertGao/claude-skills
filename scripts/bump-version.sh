...
# Single source of truth for "所有版本号" — add a Claude plugin and it is picked up
# automatically. jq is format-preserving: only the version line changes.
#
...
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)" || {
  echo "bump-version: cannot resolve repo root" >&2
  exit 1
}
cd "$root" || {
  echo "bump-version: cannot cd to repo root" >&2
  exit 1
}
die() {
