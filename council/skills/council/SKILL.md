...
- **round-one independent** — a seat's first position comes from a fresh context holding only the proposition, secret-safe truth-source bundle and persona. An `Agent` dispatch to `council:seat` provides that; a fork does not. Capacity batches are allowed when every prompt was frozen before any return. Later rounds re-dispatch a fresh worker carrying the same persona and that seat's prior boundary-compliant return; audited mode uses `council:seat`, while advisory mode keeps the least-capable fresh route available. `SendMessage` is advisory-only because §6 cannot enumerate it.
- **assurance mode** — **audited** when Claude's tool-less `council:seat` workers and JSONL/sidecar records satisfy §6, otherwise **advisory** when any fresh worker can keep round-one seats separate. Assurance changes what the result certifies, never whether the expert debate itself has value.
...
- **Advisory eligibility**: a fresh worker exists but one or more isolation, provenance or audit primitives do not. **A tool-capable fresh worker is allowed in advisory mode when no stricter worker exists; instruct it to use no tools or descendants, and disclose the missing containment.** Prefer `council:seat`, then the least-capable fresh worker available. An observed read-only seat tool call makes that return non-compliant; an observed write, network/external action or descendant dispatch triggers `STOPPED (advisory side effect detected)`. Record every missing guarantee separately. For Git-only checks, put `workdir` outside the repo and, with Bash `set -euo pipefail`, snapshot before round one and after every dispatch batch (including later rounds): `git status --porcelain=v1 -z -uall`; `git diff --binary --no-ext-diff --no-textconv`; `git diff --cached --binary --no-ext-diff --no-textconv`; and `git ls-files --others --exclude-standard -z | while IFS= read -r -d '' p; do printf '%s\0' "$p"; git hash-object --no-filters -- "$p" | tr '\n' '\0'; done`. Save outputs separately and byte-compare all four to the baseline. This sees dirty-baseline and non-ignored untracked-content net changes, not reverted, ignored-path or outside-Git writes. A post-dispatch command failure or delta ⇒ `STOPPED (advisory side effect detected)`: paste evidence and never auto-revert. A baseline failure degrades to `Soft checks: none`.
- No fresh-context worker ⇒ `STOPPED (cannot run expert seats)`. A deliberation that itself requires external or irreversible action ⇒ `STOPPED (advisory is analysis-only)`.
...
- Gap ids are normative: `prompt-provenance` prompts not canonical; `return-provenance` returns not canonical; `model-census` models unavailable; `tool-write-audit` the seat tool surface is not structurally empty or tool use is not fully recorded; `dispatch-topology` descendants are available or not excluded; `round-one-simultaneity` batching required; `auditor-re-run-capability` no fresh auditor or pinned procedure; `confirmation-provenance` presentation/response not auditable.
...
Pick candidates per axis; read each frontmatter and judge whether it fits that axis — a selection judgment, not a mechanical gate. Reject any candidate carrying the reserved persona delimiters before dispatch. Every audited seat-facing dispatch (seat, DA, cross-exam, label, compare and tie-breaker) uses `subagent_type: council:seat`; `general-purpose` is only for the auditor. Advisory mode prefers `council:seat` but may use the least-capable fresh worker available, with the no-tools/no-descendants instruction and containment gaps disclosed. Echo the seats in the fixed format:
